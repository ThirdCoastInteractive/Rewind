...
func commentCatchupLoop(ctx context.Context, dbc *db.DatabaseConnection, encMgr *encryption.Manager, ytdlpPath string) {
	select {
	case <-ctx.Done():
		return
	case <-time.After(30 * time.Second):
	}
	runCommentCatchupUnit(ctx, dbc, encMgr, ytdlpPath)
...
func runCommentCatchupUnit(ctx context.Context, dbc *db.DatabaseConnection, encMgr *encryption.Manager, ytdlpPath string) {
	q := dbc.Queries(ctx)
	...
	for _, r := range rows {
		...
		fetchAndIngestComments(ctx, q, encMgr, ytdlpPath, r.ID, r.Src, r.ArchivedBy)
		select {
		case <-ctx.Done():
			return
...
func fetchAndIngestComments(ctx context.Context, q *db.Queries, encMgr *encryption.Manager, ytdlpPath string, videoID pgtype.UUID, src string, archivedBy pgtype.UUID) {
