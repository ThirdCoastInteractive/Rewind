...
const claimVideosForCommentCatchup = `-- name: ClaimVideosForCommentCatchup :many
UPDATE videos
...
type ClaimVideosForCommentCatchupRow struct {
...
//	)
//	RETURNING id, src, archived_by
func (q *Queries) ClaimVideosForCommentCatchup(ctx context.Context, batchSize int32) ([]*ClaimVideosForCommentCatchupRow, error) {
	rows, err := q.db.Query(ctx, claimVideosForCommentCatchup, batchSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []*ClaimVideosForCommentCatchupRow
	for rows.Next() {
		var i ClaimVideosForCommentCatchupRow
		if err := rows.Scan(&i.ID, &i.Src, &i.ArchivedBy); err != nil {
			return nil, err
		}
...
const filterExistingVideoIDs = `-- name: FilterExistingVideoIDs :many
SELECT id
FROM videos
WHERE id = ANY($1::uuid[])
`
...
//	FROM videos
//	WHERE id = ANY($1::uuid[])
func (q *Queries) FilterExistingVideoIDs(ctx context.Context, ids []pgtype.UUID) ([]pgtype.UUID, error) {
	rows, err := q.db.Query(ctx, filterExistingVideoIDs, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) ListVideosMissingVideoPath(ctx context.Context, limit int32) ([]string, error) {
	rows, err := q.db.Query(ctx, listVideosMissingVideoPath, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
