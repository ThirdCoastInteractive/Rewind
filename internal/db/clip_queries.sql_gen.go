...
const updateClipShotList = `-- name: UpdateClipShotList :exec
UPDATE clips
SET shot_list = $1,
    updated_at = NOW()
WHERE id = $2
...
type UpdateClipShotListParams struct {
...
//	    updated_at = NOW()
//	WHERE id = $2
func (q *Queries) UpdateClipShotList(ctx context.Context, arg *UpdateClipShotListParams) error {
	_, err := q.db.Exec(ctx, updateClipShotList, arg.ShotList, arg.ID)
	return err
}
