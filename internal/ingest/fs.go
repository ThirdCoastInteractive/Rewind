...
// moveVideoToPermanentStorage moves video files from spool to DOWNLOADS_DIR/{videoID}/
...
func moveVideoToPermanentStorage(ctx context.Context, videoID, spoolDir string) (*string, *string, *string, *int64, error) {
	permanentDir := filepath.Join(downloadsDir(), videoID)
	...
	for _, srcPath := range files {
		...
		if isVideo && preferredVideoSrcPath != "" && srcPath != preferredVideoSrcPath {
			// Keep alternate source downloads in the spool for recovery.
			slog.Info("retaining alternate source video", "path", srcPath)
			continue
		}
		...
		if err := os.Rename(srcPath, destPath); err != nil {
			...
			if err := copyFile(srcPath, destPath); err != nil {
				return nil, nil, nil, nil, fmt.Errorf("move %s to %s: %w", srcPath, destPath, err)
			}
			_ = os.Remove(srcPath)
		}
	...
	_ = os.Remove(spoolDir) // Only remove an empty directory; retain unprocessed files.
	...
	if err := persistDirToBlob(ctx, videoID, permanentDir); err != nil {
		return nil, nil, nil, nil, fmt.Errorf("persist to blob: %w", err)
	}
...
func isCanonicalVideoDir(dir string, videoID string) bool {
	...
	if filepath.Clean(dir) == filepath.Clean(filepath.Join(downloadsDir(), videoID)) {
		return true
	}
...
func migrateVideoAssetsToCanonicalDir(ctx context.Context, videoID string, videoPath string, thumbnailPath *string) (string, *string, error) {
	...
	newDir := filepath.Join(downloadsDir(), videoID)
