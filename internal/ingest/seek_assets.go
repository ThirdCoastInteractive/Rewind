...
func seekLevelsFromEnv(contexts ...context.Context) []seekLevelSpec {
	...
	if v := strings.TrimSpace(runtimecfg.Env(ctx, "SEEK_ENABLE_XFINE")); v == "1" || strings.EqualFold(v, "true") {
	...
	if v := strings.TrimSpace(runtimecfg.Env(ctx, "SEEK_ENABLE_XXFINE")); v == "1" || strings.EqualFold(v, "true") {
		levels = append(levels, seekLevelSpec{
			Name:            "xx-fine",
	...
	if v := strings.TrimSpace(runtimecfg.Env(ctx, "SEEK_ENABLE_XXXFINE")); v == "1" || strings.EqualFold(v, "true") {
		levels = append(levels, seekLevelSpec{
			Name:            "xxx-fine",
			IntervalSeconds: 0.1,
			ThumbWidth:      80,
