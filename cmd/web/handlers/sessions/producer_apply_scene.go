...
func HandleProducerApplyScene(sm *auth.SessionManager, dbc *db.DatabaseConnection, hub *producer.SceneHub) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// Build scene from form with epoch_ms timestamp
		scene := buildSceneFromFormWithEpoch(c)
		...
		if err := dbc.Queries(ctx).UpdatePlayerSessionState(ctx, &db.UpdatePlayerSessionStateParams{State: newState, ID: session.ID}); err != nil {
			...
			return c.String(500, "Failed to apply scene")
		}
...
// HandleProducerApplyScenePreset applies a saved preset to the session.
