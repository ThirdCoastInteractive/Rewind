...
func scheduleDueWatches(ctx context.Context, dbc *db.DatabaseConnection) error {
	...
	for _, w := range watches {
		...
		if err != nil {
			// Schedules are validated at create/update; if one rots anyway,
			// retry hourly rather than spinning on it every tick.
...
func processWatchScanJob(ctx context.Context, dbc *db.DatabaseConnection, q *db.Queries, client *ytdlp.Client, job *db.DownloadJob) error {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			// Watch deleted between scheduling and processing; nothing to do.
			zero := int32(0)
	...
	if err := runWatchScan(ctx, dbc, q, client, job, watch); err != nil {
		// A foreign-key violation on the seen-ledger means the watch was
		// deleted while the scan was running. The write transaction rolled
		// back (no children were enqueued): finish quietly, not as a failure.
...
func runWatchScan(ctx context.Context, dbc *db.DatabaseConnection, q *db.Queries, client *ytdlp.Client, job *db.DownloadJob, watch *db.WatchedChannel) error {
	...
	// The first scan always walks the channel's whole catalog (metadata only)
	// so the ledger fingerprints every existing video; backfill just decides
	// whether that catalog is downloaded or only recorded. Routine scans then
	// only need the newest watchScanDepth entries per tab.
	deep := firstScan
	...
	// First useful scan: adopt the channel's own title if the user left the
	// label blank.
	...
	// Canonical domain for deterministic video UUIDs (must match ingest).
	...
	// Escalate: a routine scan whose entire head window is unseen may be
	// missing older uploads beyond it (e.g. a burst of more than
	// watchScanDepth uploads since the last scan). Re-enumerate the whole
	// catalog once — safe for every watch, since the first scan fingerprinted
	// the full back catalog into the ledger.
	if !deep && !firstScan &&
	...
	// A non-backfill first scan only seeds the seen-ledger: everything
	// currently on the channel is marked seen without being downloaded.
	...
	// Write phase runs in one transaction, ledger first: the ledger inserts
	// take FK share-locks on the watch row, so a concurrent watch deletion
	// either waits for the commit or aborts the whole batch — a half-deleted
	// watch can never enqueue downloads (the cascade would otherwise wipe the
	// ledger mid-scan and make the entire channel look new).
	...
	// Ledger updates: fresh entries (enqueued or seeded), plus entries that
	// were archived through other paths — recording those too keeps future
	// scans on the fast seen-ledger path.
	...
	// Keep the jobs page signal-only: scans that found nothing new (the
	// overwhelmingly common case) are auto-archived.
	if found == 0 {
		_ = q.ArchiveJob(ctx, job.ID)
	}
