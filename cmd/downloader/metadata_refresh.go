...
func metadataRefreshLoop(ctx context.Context, dbc *db.DatabaseConnection, encMgr *encryption.Manager) {
	...
	runMetadataRefreshUnit(ctx, dbc, encMgr)
...
func runMetadataRefreshUnit(ctx context.Context, dbc *db.DatabaseConnection, encMgr *encryption.Manager) {
	q := dbc.Queries(ctx)
	...
	for _, r := range rows {
		...
		refreshVideoMetadata(ctx, q, encMgr, r)
		select {
		case <-ctx.Done():
			return
...
// for 7 days).
func refreshVideoMetadata(ctx context.Context, q *db.Queries, encMgr *encryption.Manager, row *db.ClaimVideosForMetadataRefreshRow) {
	...
	// Chapter/description auto-markers: comments.IngestFromInfoJSON already
	// runs this when comments are present; call it here so chapters still
	// update on comment-less refresh.
